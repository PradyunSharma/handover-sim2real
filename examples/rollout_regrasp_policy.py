...
from handover_sim2real.regrasp import viz as _viz
...
# The bin colours and the three conditioning overlays now live in
# `handover_sim2real/regrasp/viz.py`, because `visualize_bc_dataset.py` draws the
# same three things for RECORDED demonstrations and the two views are only
# comparable by eye if they are drawn identically. Re-exported under the old
# names so nothing below this point had to change.
