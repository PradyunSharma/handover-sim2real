...
emits K candidates per scene, in farthest-point-sampling order:
    ...
    slot 1.. greedily the grasp furthest from everything chosen so far, under
             the flip-invariant control-point metric in `regrasp/grasp_select.py`.
...
here, collect demonstrations for all K, and let `select_regrasp_demos.py` choose
...
and returns four poses that are really two. See `regrasp/grasp_select.py`.
...
from handover_sim2real.regrasp import build_sim_cfg, build_sim_context
from handover_sim2real.regrasp.env_setup import preflight
from handover_sim2real.regrasp.grasp_select import (
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--cfg-file", default="examples/configs/regrasp_run1.yaml",
...
def main() -> None:
    ...
    for sc in range(n):
        ...
        # full set. Keep the entry anyway — select_regrasp_demos.py is the single
    ...
    print(f"  fewer than {args.n_final} distinct : {n_short}  "
          f"(these cannot survive select_regrasp_demos.py)")
    if counts:
        c = np.asarray(counts)
