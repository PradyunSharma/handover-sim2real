...
try the next pinned grasp from there.
    ...
    python examples/eval_chained_retry_p5.py \\
...
from handover_sim2real.dagger5 import load_policy_runner            # noqa: E402
from handover_sim2real.dagger5.chained_retry import (               # noqa: E402
    BUDGETS, REWIND_MODES, RetryParams, chained_metrics, chained_retry_scene,
)
from handover_sim2real.dagger5.setup import build_phase5_context    # noqa: E402
...
def write_h5(path: Path, per_scene: dict, save_clouds: bool) -> tuple[int, int]:
    ...
    with h5py.File(path, "w") as f:
        f.attrs["schema"] = "phase5_chained_retry"
...
def main() -> None:
    ...
    # to be in the SIM block BEFORE build_phase5_context — flipping cfg.SIM.RENDER
    ...
    ctx = build_phase5_context(cfg5, seed=seed)
    ...
    viz = None
    if args.render:
        from handover_sim2real.dagger5.chain_viz import ChainViz
