...
Plot a Phase-5 DAgger run from its dagger_log.csv.
    ...
    python examples/plot_dagger_run_p5.py output/dagger_runs/dagger5_run1 --show
...
train_dagger_phase5.py) and renders three figures.
...
PHASE 5 (<run>/curves_p5.png) — 1x3, "is the conditioning doing anything":
...
def main() -> None:
    ...
    # ── PHASE 5 (<run>/curves_p5.png) — "is the conditioning doing anything" ──
    ...
    if _finite(num("cond_track")) or _finite(num("retry_at_2")):
        ...
        p5_out = run / "curves_p5.png"
        fig3.savefig(p5_out, dpi=140)
        print(f"wrote {p5_out}")
