...
strictly stronger statement and the one worth localising. `--scenes eval` gives
the honest generalisation number; `--scenes all` gives both pools together.
...
def main() -> None:
    ...
    print("=" * 78)
    print(f"Per-scene eval   run={run_root.name}   pool={args.scenes} "
          f"({len(scenes)} scenes)")
    ...
    prefix = Path(args.out_prefix) if args.out_prefix \
        else run_root / f"scene_eval_{args.scenes}"
    prefix.parent.mkdir(parents=True, exist_ok=True)
    write_scene_csv(prefix.with_suffix(".csv"), rows)

    s = summarise(m, rows, scenes, args.scenes, run_root, ckpt_dir, ckpt)
    s["eval_s"] = round(dt, 1)
