...
what the training set contains:
    ...
    build_direction_table.py   K=8 diverse candidates per scene
    collect_regrasp_demos.py      one demonstration per (scene, candidate)
    select_regrasp_demos.py          <- keep the N=4 most separated that WORKED
...
`collect_regrasp_demos.py` appends the closure transition only `if not done`.
