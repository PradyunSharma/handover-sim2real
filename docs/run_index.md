...
| **regrasp_run21** | `regrasp_run21.yaml` | run 19 | wrist+right | 25 (planned) | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | **dequantized** (continuous) | train: grasp axis `−R[:,2]` / deploy: BIN CENTROID | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / **20** | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256], **shield on** | not yet run | not yet run | = run 19, **TWO expert-side bug fixes**, one of which needs no config key. **(1) `DAGGER.replan_after_pin: true`** — the DAgger collector took `expert_target_jp = plan[0]` from the UNPINNED plan, then pinned the goal set and updated `grasp_pose`/`standoff_pose`/`d_world`, and never recomputed the label, so at step 0 the caption, the command and the expert action could name different grasps. `collect_regrasp_demos.py` always did it right (plan → pin → replan `reset_scene=False` → replay). Scene 95 grasp 2, identical starts to 0.00000 m and identical grasps to 0.0000 m / 0.0°: base step-0 label 120.8° off-axis moving 4 cm AWAY (the lateral arc the scene needs), DAgger's 61.0° closing monotonically — straight in, which is what hit the object. OMG is deterministic here (8 episodes from iters 1–19, 0.0° spread), so this is not planner noise. Read `pin_goal_moved / episodes` in dagger_log.csv: it counts episodes where the goal INDEX moved across the pin, i.e. where the old label was genuinely wrong. **(2) the pool fix, NO config key** — `build_regrasp_context` never applied the reach prune that `train_regrasp.py` applies to the manager's copy, and `keep_only` RENUMBERS SLOTS, so the manager and its twenty workers disagreed about what every slot index MEANS while the pool is shipped `(scene, slot)` and not the table. Measured on run 19: **246 of 1097 manager slots (22.4%) named a different bin in the worker**, and **898 of 4883 episodes (18.4%) landed on pairs the reach filter had excluded**; those failed `reached()` **77.1%** of the time vs **31.1%** for pairs that belonged in the pool, so they are 692 of run 19's 1933 dropped episodes — **36% of all the collection waste**. `eval_jobs` ships slots the same way, so per-bin scoring was mis-attributed on the same ~22% of slots. **Scene 79 is the bug in one scene**: base bin 0 (+x) stopped 3.55 cm short, base bin 4 (+z) reached to 0.46 cm, and over 25 iterations DAgger collected bin 0 **three times** and bin 4 **zero** — the manager's one-slot table only ever emits slot 0, which is bin 0 in the worker's two-slot table, so bin 4 sat at an index the manager could never emit. Fixed in `handover_sim2real/regrasp/setup.py`; `eval_regrasp_testset.py` turns it off on test/val, where it is as split-dependent as `demo_ok_table`. **Run 19's numbers are not reproducible on this code — that is the point.** NOT a clean ablation (neither was run 19); both changes correct the EXPERT side and isolating them costs a second ~17 h run. Expect `reached_grasp/episodes` to rise from run 19's 0.604 toward ~0.69 on arithmetic alone, before any policy gain; noise floor 0.088, so only a success-rate gain past ~0.09 is readable. `EVAL.holdout` still false. |
