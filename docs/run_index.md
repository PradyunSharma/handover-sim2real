...
| **dagger4_run21** | *queued* | | = run 16 with **the reach made open loop**, as in the CVPR2023 paper: `DAGGER.target: grasp → pregrasp`. The policy is asked for the **approach only**; its channel-6 zero stops meaning "close the fingers" and starts meaning **"commit"**, on which the arm pushes **6.4 cm along its own local +z and closes, blind** — no observation, no grasp pose. That is `HandoverSim2RealPolicy.grasp_and_back` (`policy.py:273`), implemented as `dagger/pregrasp.py`. **Collection**: the CLOSE label fires within the *same* (0.02 m, 0.34 rad) tolerances of the **standoff** instead of the grasp, and the episode ends there; the committed reach degenerates to holding the standoff waypoint, so every rule keyed on `committed_reach is not None` (DART band floor, reach-jolt + its servo-back target, `expert_after_commit`) carries over untouched. **Eval**: the commit triggers the same blind push, *then* the hold — `success_rate` is the identical `grasp_held_after_hold` call, so the headline number stays directly comparable to run 16's even though the policy produces 6.4 cm less of the motion. **Schema unchanged** — still [T,1024,5] / [T,32] / [T,7] with a binary channel 6 and one terminal 0; no model, loss, dataset or normalizer code touched. **Why**: the reach is the part runs 1–20 could not learn *and* the part that needs no learning — a straight line whose length is a planner constant, `standoff_dist·(1 − 1/reach_tail)`. **D₀ is a prefix, not a re-collection** (`examples/make_pregrasp_demos.py`): the base collector plays the plan by index and appends one close, so the state recorded after waypoint `len(plan) − reach_tail` *is* the state at the standoff with the cloud it observed there; keep rows 0…`len(plan)−reach_tail+1`, rewrite that last action as CLOSE, 21 steps → 17. Clouds, states and approach labels are **bit-identical to run 16's**, so a difference in outcome cannot be a difference in data. **What it costs, measured** over the 472 kept demos: the displacement from the new terminal state to the state the expert closed at is **0.0642 m axial** (median 0.0652, p90 0.0731) — which is why `forward_dist` is 0.064 and *not* the paper's 0.08, that would overshoot by 1.6 cm — plus **0.0088 m lateral** (p90 0.0152, max 0.0418) and 0.027 rad of rotation. The lateral column is the price: off-axis correction the blind push cannot make. Inside `close_pos_thresh`, but it is the number to weigh a drop in success against. **Read `mean_min_pos`** (now distance to the *pre-grasp*, should fall sharply), **`c_reach_pos` / `mean_reach_pos_err`** (where the blind push landed vs the grasp — a small `mean_min_pos` with a large reach error is a `forward_dist` to re-tune, **not** a failed run), and **`box_after_rate`** (of the commits, how many put the object in the open jaws). `box_chance_rate` is ≈0 **by construction** here and is not a regression. Also `derive_standoff: true` (the standoff is now the *terminal condition*, so it must exist at reset and survive a failed replan) and `outcome_check: true`, now standard. **The interaction to watch**: `expert_after_commit: true` is inherited unchanged but now covers only the 5 cm to the standoff rather than run 16's ~11.4 cm through the grasp — and that is exactly the arrival precision the policy is now meant to own, so it is the first follow-up knob, along with re-deriving `reach_tail_weight: 2.5` (positional, so it still up-weights the terminal region, but its derivation assumed 1.2 cm reach steps that no longer exist). |
