...
Phase-4 one, plus seven this phase needs: `config`, `max steps` (the per-episode
step budget a rollout gets), `dirs/scene` (how many
directions a scene demonstrates), `command` (what vector the policy is actually
told), `d injection` (where `d` enters the network), `d noise` (`DATA.d_noise_deg` in the learner config — how far the
...
**`d injection` is `per-point ×2` in every run of the phase**, and that is a
deliberate architectural bet rather than an oversight. `d` never reaches the
network as a vector. It is consumed into two extra per-point channels
(`regrasp/channels.py::append_direction_channels`) —

    d · n_i                    is this surface facing the incoming gripper?
    d · normalize(p_i − c)     is this point on the side I am approaching from?

— taking `pc_channels` from 5 to 7, and Phase 5's `GraspEncoder` (an MLP whose
embedding was concatenated into the fused vector) was **deleted** rather than
repointed, so `fused_dim` is back to `2 * feature_dim` and there is no global
branch to concatenate into. Every run is therefore `MODEL.direction_cond: true`
across all four learner configs, with no key that would turn a global branch on.

THE THIRD VALUE OF THIS COLUMN IS THE OBVIOUS NEXT RUN. The SA layers max-pool,
and two scalars among ten input features can be washed out — which is exactly
what `dir_err` at a 36–46° median across runs 10–12 looks like. Putting a small
global `d` branch back alongside the per-point channels would make the column
read `both`, and the gap between that run and its base is the measurement of how
much the pooling was destroying. The note reserving that spot is in
`regrasp_bc/models.py:150`.

**`max steps` is 50 in every run of the phase** — `DAGGER.max_steps` and
`EVAL.max_steps` are set equal in all 17 configs, so collection and scoring give
an episode the same budget and the column is a constant rather than a
comparison. It is here because it is *not* the default anywhere else: the
`rollout_regrasp_policy.py` viewer used to open at 30, which silently clipped
episodes — `mean_close_step` is 25.6 on run 10 at iteration 21 — and made the
GUI disagree with `eval_log.csv` on the same checkpoint and scene. Any future
run departing from 50 makes this column earn its place; until then it is a
pinned constant. Note the budget is in POLICY steps, and one policy step is
`TIME_ACTION_REPEAT` = 0.15 s of simulation, so 50 steps is 7.5 s.

**The five identity columns**, all read from the configs rather than the prose,
so a row can be classified without opening anything:

| column | key | values |
|---|---|---|
| `base run` | — | the run this one was derived from. Every Δ in `notes` is relative to it. |
| `` `d` means `` | `SIM.d_rule` | **orientation** = `approach_axis`, `d = −R_grasp[:,2]`, which way the gripper POINTS. **direction** = `grasp_offset`, `d = normalize(fingertip − centroid)`, which SIDE of the object it closes on. |
| `label` | `DATA.d_source` | **quantized** = `d_world`, the commanded vector, i.e. `d` snapped to one of the k bin axes (or the bin's empirical centroid). **dequantized** = `d_grasp_world`, the continuous direction of the grasp the expert actually flew. |
| `warm start` | `TRAIN.train_from_scratch` | **no (FTL)** = every iteration refits the aggregate from a random init, which is Follow-The-Leader and paper-faithful. **yes** = each iteration initialises from the previous one's checkpoint, named in the cell (`TRAIN.init_ckpt`). |
| `DART mode` | `DAGGER.dart_mode` | **`jolt` — replace** = the expert action is REPLACED by a uniform box draw on `dart_ratio` of steps (runs 1–11). **`dart_noise` — add** = `ε ~ N(0, Σ)` is ADDED to `π*(s)`, Σ measured and trace-rescaled per the paper (runs 12–15). |

Two cautions on those. `` `d` means `` and `label` are independent — run 10 is
`direction` + `quantized`, run 9 is `orientation` + `dequantized` — and the pair
is what a row's conditioning actually is; neither alone identifies it. And
`` `d` means `` changes what the *evaluation* measures, not just the training.
Under `direction`, `d` is a lever arm of median 3.9 cm, so a placement error
rotates it: measured on the demonstrations, stopping 5 cm short of the grasp with
the orientation UNCHANGED flips the realised bin 51.8% of the time under
`grasp_offset` and **0.0%** under `approach_axis`. So `bin_diag_rate` collapses to
chance at ~5 cm of arrival error on a `direction` run while an `orientation` run
still reads 0.90 there. Rows with different `` `d` means `` are **not** comparable
on `bin_diag_rate`, `dir_err`, `sector_err` or `bin_hit_rate` — a lower number is
as likely to mean "arrived less often" as "followed the command less well".
...
| run | config | base run | camera | iters | m | max steps | dirs/scene | `d` means | label | command | d injection | d noise | init | warm start | epochs (base/iter) | β | DART free | DART reach | DART mode | DART σ pos/rot (free → reach) | aux task | loss | model flags | best iter | final | notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **regrasp_run1** | `regrasp_run1.yaml` | — | wrist+right | **19 of 25** | 354 → 176 | 50 | 1–2 (max-separated pair) | orientation (`−R[:,2]`) | quantized (bin axis) | grasp axis `−R[:,2]` | per-point ×2 | 12° | warm-start (**best**.pt), PointNet++ from scratch | **yes** (best.pt) | 40 / 12 | constant 0.75 | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | no | pm (w=7) | nojoint, reach-tail ×2.5 (window **5**), **direction_cond**, head [256,256] | 8 (`success 0.354`) | `success 0.139` | First Regrasp run. Stopped at 19 by choice; `m` changed mid-run, so `D_steps` kinks at it 6. See below. |
| **regrasp_run2** | `regrasp_run2.yaml` | run 1 | wrist+right | **19 of 25** (running) | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | quantized (bin axis) | **bin axis** | per-point ×2 | 12° | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | **yes** (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | **13** (`success 0.563`) | it19 `success 0.487`, `dir_track 0.722`, `bin_diag 0.924` | = run 1, protocol fixed + 2 method changes. **Best run in the phase**: `success 0.143 → 0.563`, `bin_diag 0.971` @it16 vs chance 0.25. See below. |
| **regrasp_run3** | `regrasp_run3.yaml` | run 2 | wrist+right | 25 (planned) | 1200 → ~742 | 50 | **1–4 bins × 3 grasps** (~7.4 slots) | orientation (`−R[:,2]`) | quantized (bin axis) | **bin axis** | per-point ×2 | 12° | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | not yet run | not yet run | = run 2 with **three demos per bin**; `m` 400 → 1200 to hold 100 scenes. Base set 1596 → 4578. See below. |
| **regrasp_run4** | `regrasp_run4.yaml` | run 3 | wrist+right | 25 (planned) | 1200 → ~742 | 50 | **1–4 bins × 3 grasps** (~7.4 slots) | orientation (`−R[:,2]`) | quantized (bin axis) | **bin axis** | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.5** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | not yet run | not yet run | = run 3, noise off (`d_noise_deg` 12 → 0) **and** `beta_end` 0.75 → 0.5. Two changes — **not a clean ablation**. See below. |
| **regrasp_run5** | `regrasp_run5.yaml` | run 3 | wrist+right | 25 (planned) | 1200 → ~742 | 50 | **1–4 bins × 3 grasps** (~7.4 slots) | orientation (`−R[:,2]`) | quantized (bin axis) | **bin axis** | per-point ×2 | 12° | **warm-start (best.pt)**, PointNet++ from scratch at iter 0 only | **yes** (best.pt) | 50 / 15 | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | not yet run | not yet run | = run 3 + warm start. ONE change — the clean ablation run 4 is not. See below. |
| **regrasp_run6** | `regrasp_run6.yaml` | run 4 | wrist+right | 25 (planned) | **7404 → ~4578 (ALL 617 scenes)** | 50 | **1–4 bins × 3 grasps** (~7.4 slots) | orientation (`−R[:,2]`) | quantized (bin axis) | **bin axis** | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.5** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | not yet run | not yet run | = run 4 with `m` 1200 → **7404** = every scene every iteration. **~223 h GPU / 47 GB**; `num_iters: 8` gets most of it. See below. |
| **regrasp_run7** | `regrasp_run7.yaml` | run 2 | wrist+right | **17 of 25** (running) | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | quantized (bin axis) | **bin axis** | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.5** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | **12** (`success 0.555`) | it17 `success 0.496`, `dir_track 0.704`, `bin_diag 0.903` | = run 2 + run 4's two changes. **The noise transformed the base fit and nothing after it** — indistinguishable from run 2 by it 7. See below. |
| **regrasp_run8** | `regrasp_run8.yaml` | run 7 | wrist+right | **19 of 25** (running) | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | quantized (bin axis) | **bin axis** | per-point ×2 | **0° (OFF)** | **warm-start (best.pt)**, PointNet++ from scratch at iter 0 only | **yes** (best.pt) | 50 / 15 | **linear 0.9→0.5** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | **14** (`success 0.487`) | it19 `success 0.475`, `dir_track 0.704`, `bin_diag 0.908` | = run 7 + warm start. **Removes the FTL dip and buys nothing.** Its it-0 gap sets the **0.088 noise floor** used everywhere here. See below. |
| **regrasp_run9** | `regrasp_run9.yaml` | run 2 | wrist+right | **25 of 25** | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | **dequantized** (continuous) | **train: grasp axis `−R[:,2]` / deploy: BIN CENTROID** | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | **23** (`success 0.592`) | it25 `success 0.496`, `eval_min_pos 0.0712`, D = 7968 eps | **First run whose training label and deployment command are different vectors.** A pure relabelling of run 2's shards. See below. |
| **regrasp_run10** | `regrasp_run10.yaml` | run 2 | wrist+right | **25 of 25** | 600 → ~187 | 50 | **1–6 (one per bin, ALL SIX LIVE)** | **direction** (centroid→tips) | quantized (bin axis) | bin axis, but **`d` = centroid → fingertip** | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | **25** (`success 0.608`) | it25 `success 0.608`, `eval_min_pos 0.1060`, D = 3635 eps | **First run to change what `d` MEANS** — position, not orientation. Unlocks `−z`/`−x` (chance 1/4 → 1/6); first under `reach_filter`. Whole upstream chain rebuilt, so **not** a single-change test. See below. |
| **regrasp_run11** | `regrasp_run11.yaml` | run 10 | wrist+right | **25 of 25** | 600 → ~187 | 50 | **1–6 (one per bin, ALL SIX LIVE)** | **direction** (centroid→tips) | **dequantized** (continuous) | **grasp's own `d`** (continuous), deploy on the **bin centroid** | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | **22** (`success 0.619`) | it25 `success 0.572`, `eval_min_pos 0.1022`, D = 3660 eps | **Label de-quantised**: the bin caption sat a median **24.5°** from the flown grasp's own `d`. Pure relabelling, no new data. See below. |
| **regrasp_run12** | `regrasp_run12.yaml` | run 11 | wrist+right | **25 of 25** | 600 → ~187 | 50 | **1–6 (one per bin, ALL SIX LIVE)** | **direction** (centroid→tips) | **dequantized** (continuous) | **grasp's own `d`** (continuous), deploy on the **bin centroid** | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / 15 | **linear 0.9→0.75** | 0.3 | 0.3 | `dart_noise` — add  ⚠ **mis-dosed** (α 1.0→0.25, ratio 1.0) | **MEASURED, and larger than intended**: it 2 **2.7–3.6 cm / 0.09–0.13 rad** → reach 0.8–1.1 cm / same; it 25 **1.4–1.8 cm / 0.05–0.07 rad** → reach 0.4–0.5 / same. The α table was anchored to phase 4's tr(Σ̂)=0.0161; this stack measures **0.0408**, so σ ran **1.59×** the design and above the jolt for the first half. Fired on **90%** of all steps vs the jolt's 26%. | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256] | **22** (`success 0.562`) | it25 `success 0.490`, `eval_min_pos 0.1117`, D = **2147** eps (run 11: 3660) | DART as the paper writes it. **Ran and regressed** (0.562 vs run 11's 0.619): ratio 1.0 perturbed 90% of steps, so the reach never committed. See below. |
...
| **regrasp_run19** | `regrasp_run19.yaml` | run 9 | wrist+right | 25 (planned) | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | **dequantized** (continuous) | train: grasp axis `−R[:,2]` / deploy: BIN CENTROID | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / **20** | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256], **shield on** | not yet run | not yet run | ⚠ **SEE "The reach filter never reached the workers" under Standing conclusions** — run 19 is the run the pin-table bug was measured on, and 18.4% of its DAgger episodes were collected on pairs the reach filter was supposed to exclude. Fixed for run 21; run 19's own shards are not repairable. <br>= run 9, **TWO changes — not a clean ablation**: `DAGGER.shield: true` (collection) and `TRAIN.iter_epochs` 15 → **20** (fitting). A per-step collision veto on BOTH bands and BOTH drivers. A refused JOLT is redrawn (≤5 tries, then no jolt); a refused POLICY action is replaced by the expert's; the EXPERT is never vetoed. Bar is RELATIVE — `min(shield_clearance, what pi* was going to do)` — because by the close the gripper is *supposed* to be millimetres away and an absolute floor refuses the demonstration itself. Motivated by run 9's held-out failure budget: **45.9% of episodes end in a collision** (26.8% object knocked down, 19.1% hit the human) against 0.0% closed-on-nothing and 78% close→grasp conversion, i.e. the approach destroys the scene before the grasp is ever attempted (`box_chance` 44.7%). Reuses run 9's tables and base shards — nothing to rebuild. **Read `shield_policy / (steps − expert_steps)` FIRST**: the shield raises the EFFECTIVE β above the configured 0.75, and if that fraction is large this is a higher-β run rather than a shield test. Known cost: the states DOWNSTREAM of a collision are never visited, so coverage of the failure is traded for more episodes that reach the close. **The epoch change is a second variable**: 15 was inherited from run 2 and never argued for on this aggregate, while Phase-4 runs 16/19 — also FTL from scratch, and the two that transfer best to the robot — use 25 on a 100-epoch base. Run 9 goes flat from iteration 14 while D grows 5174 → 7968, the signature of a refit that cannot absorb its data. A gain here is not attributable without `run 9 + shield only` or `run 9 + epochs only`. Budget ~16-17 h against run 9's measured 13.4 h. |
| **regrasp_run20** | `regrasp_run20.yaml` | run 19 | wrist+right | 25 (planned) | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | **dequantized** (continuous) | train: grasp axis `−R[:,2]` / deploy: BIN CENTROID; frame azimuth from the **ROBOT BASE** | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / **20** | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256], **shield on** | not yet run | not yet run | = run 19, **ONE change**: `SIM.anchor_hand_ref: wrist → base`, so `x = horizontal(p_base − c)` with z world up and the giver out of the frame entirely. **Run 20 against run 19 is the single-variable test of the frame.** Motivation is DEPLOYMENT, not sim: runs 1–19 name their bins in the MANO-wrist frame, the rig has no MANO, and `my_regrasp_policy_runner` substitutes the base — applying wrist-frame `command_axes` under a base-frame rotation. Run 9 on hardware showed the signature: the per-bin ORDERING did not match sim (+y best on the rig, +x worst; sim has +x 0.532 best, −y 0.290 worst), and a uniform sim2real loss cannot reorder bins. **EXPECT A FLAT SIM RESULT** — only 7.7% of grasps change bin, so ~92% of commands are the same vector; flat is the GOOD outcome because deployability comes free. Lever arm 61.3 cm (floor 41.8) vs the wrist's 15.7, so the hysteresis is unreachable and skipped (`mode: base_primary`). `anchor_update` stays `latched` deliberately — `live` is nearly free under this frame (1.49° drift/episode) and is the obvious next run, but would be a second variable. Full upstream rebuild under `_aabase` (approach_axis + base), kept distinct from run 18's `_bframe` (same frame, `grasp_offset` substrate). ~23–24 h total; `regrasp_run20_all.sbatch` is idempotent, expect two submissions. |
| **regrasp_run21** | `regrasp_run21.yaml` | run 19 | **right ONLY** (no wrist) | **35** (planned) | 400 → ~259 (pool **+18%** effective, see notes) | **80** | **1–4 (one per bin)** | orientation (`−R[:,2]`) | **dequantized** (continuous) | train: grasp axis `−R[:,2]` / deploy: BIN CENTROID | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / **20** | **linear 0.9→0.75 over 35** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256], **shield on**, **plane gate on**, **replan_after_pin** | not yet run | not yet run | = run 19, **SEVEN changes**: two bug fixes, four behaviour changes, one rig change. **NOT a clean ablation** — neither was run 19, which moved the shield and `iter_epochs` together. <br>**(1) `DAGGER.replan_after_pin: true`** — the collector took `expert_target_jp = plan[0]` from the UNPINNED plan, then pinned the goal set and updated `grasp_pose`/`standoff_pose`/`d_world`, and never recomputed the label, so at step 0 the caption, the command and the expert action could name different grasps. `collect_regrasp_demos.py` always did it right (plan → pin → replan `reset_scene=False` → replay). Scene 95 grasp 2, identical starts to 0.00000 m and identical grasps to 0.0000 m / 0.0°: base step-0 label 120.8° off-axis moving 4 cm AWAY (the lateral arc the scene needs), DAgger's 61.0° closing monotonically — straight in, which is what hit the object. OMG is deterministic here (8 episodes, iters 1–19, 0.0° spread). Read `pin_goal_moved / episodes`. <br>**(2) the pool fix, NO config key** — `build_regrasp_context` never applied the reach prune `train_regrasp.py` applies to the manager's copy, and `keep_only` RENUMBERS SLOTS, so the manager and its twenty workers disagreed about what every slot index MEANS while the pool is shipped `(scene, slot)` and not the table. **246 of 1097 manager slots (22.4%) named a different bin in the worker**; **898 of 4883 run-19 episodes (18.4%) landed on pairs the reach filter had excluded**, and those failed `reached()` **77.1%** vs **31.1%** for pairs that belonged — 692 of run 19's 1933 drops, **36% of all collection waste**. `eval_jobs` ships slots the same way, so per-bin scoring was mis-attributed on the same ~22%. **Scene 79 is the bug in one scene**: base bin 0 (+x) stopped 3.55 cm short, base bin 4 (+z) reached to 0.46 cm, and over 25 iterations DAgger collected bin 0 **three times** and bin 4 **zero** — the manager's one-slot table only ever emits slot 0, which is bin 0 in the worker's two-slot table, so bin 4 sat at an index the manager could never emit. Fixed in `regrasp/setup.py`; `eval_regrasp_testset.py` disables it on test/val, where it is as split-dependent as `demo_ok_table`. **Run 19's numbers are not reproducible on this code — that is the point.** <br>**(3) `DAGGER.plane_gate: true`** — a plane AT the standoff, normal along the gripper's +z at the pre-grasp (the approach axis): free approach one side, reach tail the other. A DART jolt that would cross is REDRAWN (same `shield_jolt_max_tries` budget), a policy step that would cross is replaced by the expert action — same two remedies as the shield, different predicate, and the RECORDED LABEL untouched either way. **Why a hard gate:** past this plane nothing can repair the trajectory — `goal_set_projection` pins the last `reach_tail_length` waypoints to the IK'd straight-line reach, so the obstacle gradient there is projected out by construction, and arriving off-axis in that corridor is unrecoverable BY THE EXPERT, the one thing DAgger depends on. Scenes 6 and 79 both show the signature: the hand swings past the standoff on a wrist-pivot arc (scene 79 dives **27 cm** below the grasp, screw radius collapsing to 3.4 cm at the turnaround) and then has to climb back onto it. Scoped to `committed_reach is None`. New counters `plane_policy` / `plane_jolt_reject`; **untested at scale** — if `plane_policy` is a large share of learner-driven steps, loosen `plane_gate_margin` rather than disabling the gate. <br>**(4,5) `max_steps: 50 → 80` in BOTH `DAGGER` and `EVAL`** — run 19 hit the cap on **13.9%** of episodes and rising (12.1% iters 1–5 → 17.0% iters 21–25), and the plane gate lengthens approaches further. Eval matched deliberately: a policy taught to take 80 steps and scored at 50 is penalised for what it was trained on. **Consequence: `success_rate` is NOT comparable with runs 1–20**, which scored at 50 — use the held-out test number with a matching `--max-steps`. <br>**(6) `SIM.cfg_file: pretrain_right.yaml` — RIGHT CAMERA ONLY, no eye-in-hand.** This is what forces a **full upstream rebuild** under the `_right` suffix (direction tables, pin assignment, base shards, demo-ok), and the reason is not obvious: `build_direction_table.py` records `centroid_world` as the **OBSERVED cloud's** centroid, not the object pose, so dropping the wrist camera moves it by centimetres, moves the anchor frame, and **re-bins grasps**. **Nothing in the repo validated the camera set** — shards record `cameras` in their attrs and no loader compares them — so `preflight_regrasp.sh` check 4 now compares `cfg_file` against the table's `_meta`. Run 21 ADDS ~5 GB alongside run 19's files rather than replacing them (30 GB `/home` quota). Expect this to COST success rate in sim — every run to date leaned on the wrist view for localisation; the argument is that what remains is what the rig actually has, so **holding near run 19 on one fixed camera is the good outcome, not a regression.** <br>**(7) `DAGGER.num_iters: 25 → 35` — AND THE BETA RAMP WITH IT.** `num_iters` is the DENOMINATOR of the linear schedule (`beta(i) = start + (end-start)*(i-1)/(num_iters-1)`), so 35 does not append ten iterations at the floor, it **stretches the handover**: endpoints unchanged at 0.900 → 0.750, but beta(10) 0.860 vs run 19's 0.844, beta(20) 0.816 vs 0.781, and **beta(25) 0.794 vs run 19's 0.750** — run 21 reaches run 19's FINAL beta only at iteration 35. **Do not overlay iteration-by-iteration against run 19**; at equal index they sit at different on-policy fractions, and the comparable points are the endpoints (run 19 it25 and run 21 it35 are both beta 0.750). Motivation: run 19 was still rising when it stopped — its best iteration was its LAST (0.743 @it25) and iters 14–20 averaged 0.648, so 25 was a stopping point, not a plateau (run 9 by contrast went flat from iteration 14). <br>**Also new:** `step_drivers` `int8[T]` per episode (−1 none / 0 expert / 1 policy / 2 dart_free / 3 dart_reach / 4 shield / 5 plane_gate), index-aligned with `expert_actions`, diagnostic only — the driver split was previously only recoverable in aggregate from `expert_steps`/`n_dart`. <br>**Expect `reached_grasp/episodes` to rise from run 19's 0.604 toward ~0.69 on arithmetic alone** (the pool fix removes episodes that were failing 77% of the time) — **that is bookkeeping, not learning.** Noise floor 0.088; run 19 peaked 0.743 @it25, mean 0.648 over iters 14–20. `EVAL.holdout` still false. Pipeline: `regrasp_run21_all.sbatch` (idempotent, **32–35 h** — ~7 h upstream rebuild + ~25–28 h training, since per-iteration cost rises with \|D\| and iters 26–35 add ~10 h rather than 40% of 16 — expect **3 passes** at `--time=20:00:00`); runbook `docs/runbook_run21.md`. |
| **regrasp_run22** | `regrasp_run22.yaml` | run 19 | wrist+right | 25 (planned) | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | **dequantized** (continuous) | train: grasp axis `−R[:,2]` / deploy: BIN CENTROID | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / **20** | **linear 0.9→0.75** | 0.3 | 0.3 | `jolt` — replace | 2.31 cm / 0.173 rad → 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256], **shield on**, **plane gate on**, **replan_after_pin** | not yet run | not yet run | **THE CONTROLLED RUN.** = run 19, **THREE changes, all on the EXPERT side**, everything else byte-identical — same camera (wrist+right), `max_steps: 50` on both bands, `num_iters: 25` and therefore the same beta at every index (0.900→0.750). **Reuses run 9/19's direction table, pin table, demo-ok table and base shards unchanged — NOTHING TO REBUILD**, one `train_regrasp.sbatch` submission. <br>**(1) `DAGGER.replan_after_pin: true`** — the collector took `expert_target_jp = plan[0]` from the UNPINNED plan, then pinned and updated `grasp_pose`/`standoff_pose`/`d_world`, and never recomputed the label. Scene 95 grasp 2 (identical starts to 0.00000 m, identical grasps to 0.0000 m / 0.0°): base step-0 label 120.8° off-axis moving 4 cm AWAY (the lateral arc), DAgger's 61.0° closing monotonically — straight in, which is what hit the object. ⚠ **`pin_goal_moved` read 0 in all 18 of run 21's iterations and that number is VACUOUS** — the counter compared the goal index against a value re-baselined from the same post-pin read, and the pinned goal set holds one element, so it could never fire. `replan_pin ≈ episodes` confirms the fix itself runs. Incidence is UNMEASURED, not zero. <br>**(2) the pool fix, NO config key** — `build_regrasp_context` never applied the reach prune the manager applies, and `keep_only` RENUMBERS SLOTS, so manager and workers disagreed about what every slot index MEANS. **246/1097 manager slots (22.4%) named a different bin in the worker**; **898/4883 run-19 episodes (18.4%)** landed on excluded pairs, which failed `reached()` **77.1%** vs **31.1%** — 36% of all collection waste. Scene 79: base +x stopped 3.55 cm short, +z reached 0.46 cm, and run 19 collected +x **three times**, +z **zero**. Expect ~18% fewer episodes/iter (run 21 measured 159 vs 194) on an entirely reachable pool. <br>**(3) `DAGGER.plane_gate: true`** — plane at the standoff, normal along the gripper's +z; jolts that cross are redrawn, policy steps that cross execute the expert action instead. **Run 21 measured it and only one half behaves**: `plane_policy/learner` 7.0%→8.7% (as designed), but **54% of jolt proposals refused and `dart/steps` 0.268→0.209 — a fifth of all DART LOST**, with survivors uniform *conditioned on not crossing*, biased away from the object. Cause is geometric: `dart_rot_mag: 0.3` on a ~0.58 m EE→object lever arm swings control points ~13 cm, so near the standoff a rotational jolt crosses almost every time (jolt steps: 13.10° rotation, only 3.0 cm translation). Kept as specified here, because run 21's plane numbers come with four confounds and run 22 is the first clean read on it. **`regrasp_run23` is the alternative**: it drops the gate and instead TAPERS the free jolt from ±4 cm to ±0.8 cm as the standoff is approached, shrinking the noise rather than rejection-sampling it — no jolt lost, distribution stays symmetric. Both carry both fixes and are otherwise identical, so **run 22 vs run 23 is the head-to-head between the two mechanisms** (plus run 23's DART ratios at 0.2). <br>**WHY THIS RUN:** run 21 moved seven things at once and its numbers are unreadable against run 19 — at it18 of 35, success mean 0.424 vs run 19's 0.550, `dir_track` rising normally 0.423→0.745, `reached_grasp/ep` 0.907 vs 0.610 (mostly the horizon: cap-hit 12.8%→0.2%). **run 22 vs run 19 isolates these three; run 21 vs run 22 isolates the other four; run 22 vs run 23 isolates plane-gate-vs-taper.** <br>**Read:** `success_rate` is directly comparable with run 19 iteration-for-iteration (run 19 mean 0.550 over iters 1–18, peak 0.743 @it25; **noise floor 0.088**). `reached_grasp/episodes` should go 0.610 → ~0.69 on arithmetic alone — **bookkeeping, not learning**. Watch `dir_track` for a regression, since the plane gate reshapes DART where recovery is learned. `EVAL.holdout` still false. ~15–17 h, request 20 h. |
| **regrasp_run23** | `regrasp_run23.yaml` | run 19 | wrist+right | 25 (planned) | 400 → ~259 | 50 | **1–4 (one per bin)** | orientation (`−R[:,2]`) | **dequantized** (continuous) | train: grasp axis `−R[:,2]` / deploy: BIN CENTROID | per-point ×2 | **0° (OFF)** | **scratch (FTL)**, PointNet++ random every iter | no (FTL) | 50 / **20** | **linear 0.9→0.75** | **0.2** | **0.2** | `jolt` — replace | **tapered** 2.31→0.46 cm / 0.173→0.035 rad by standoff distance; reach 0.69 / 0.173 | yes (w=1.0) | pm (w=7) | nojoint, reach-tail ×2.5 (window 5), **direction_cond**, head [256,256], **shield on**, **replan_after_pin**, **DART taper** | not yet run | not yet run | = run 19 + **BOTH bug fixes** + **DART changes**, with `plane_gate` OFF — the taper replaces it. Wrist+right, `max_steps: 50`, `num_iters: 25` and the same beta at every index; reuses run 9/19's tables and base shards, **nothing to rebuild**. <br>**(1) `dart_ratio` and `dart_reach_ratio` 0.3 → 0.2.** Run 19's measured driver split was **expert 58.2% / DART 29.7% / policy 12.2%** — nearly a third of every episode was a random jump and the LEARNER drove one step in eight. Cutting both ratios raises the policy's share without touching beta, so more of what gets labelled is a state the policy actually chose. <br>**(2) `dart_taper_floor: 0.2`, `near: 0.05`, `far: 0.20` — the free jolt shrinks toward the standoff.** `dart_pos_mag: 0.04` is ±4 cm PER AXIS fired at full size ANYWHERE in the free approach: a nudge 40 cm out, a catastrophe 5 cm from the standoff where the reach band's own jolt is ±1.2 cm — a **3.3× step change at the commit boundary** that existed only because nothing tapered it. Both magnitudes now scale linearly 1.0 at 20 cm → 0.2 at 5 cm (±4.00→±0.80 cm, ±0.300→±0.060 rad), so the bands hand over continuously. **Rotation tapers too, and it is the term that matters**: measured on run 21 `episode_00048`, a jolt translates the hand 3.0 cm but rotates it 13.1°, and the object sits ~0.58 m from the EE ORIGIN in the EE frame, so rotation alone sweeps the observed cloud 13 cm (r·Δθ = 0.132 m vs 0.114 m measured). A "4 cm" jolt is really a 13 cm observation jump; tapering position alone would scale the small term and keep the large one. <br>**THIS IS THE PLANE GATE'S RIVAL, NOT ITS COMPANION.** Run 21 measured the gate refusing **54% of free-jolt proposals** and **losing 22% of all DART** (`dart/steps` 0.268→0.209) — the 5-redraw budget runs out and the jolt is abandoned, and survivors are uniform *conditioned on not crossing*, skewed away from the object, removing exactly the recovery states nearest the goal. A taper reaches the same end by SHRINKING the noise rather than rejection-sampling it: no jolt is lost and the distribution stays symmetric. **run 23 vs run 22 compares the two approaches**, with the caveat that run 22 also carries `replan_after_pin`. <br>**CARRIES BOTH FIXES.** `DAGGER.replan_after_pin: true` (fix 1, step-0 expert label — the collector took `plan[0]` from the UNPINNED plan then pinned and updated everything else but the label; on here because it is a correction, not a variant, and its incidence cannot be measured until `pin_goal_moved` is repaired — that counter read 0 in all 18 of run 21's iterations and is **vacuous**, comparing the goal index against a value re-baselined from the same post-pin read of a one-element goal set). Fix 2, the worker/manager pin table, is unconditional and **cannot be disabled**, so no run on this code reproduces run 19 exactly; its effects are bookkeeping — ~18% fewer episodes/iter, `reached_grasp/episodes` 0.610 → ~0.69 on arithmetic — and land on different columns from DART. **Do not attribute a reach-rate gain here to the DART settings.** <br>**What this licenses:** *run 23 vs run 22* is the head-to-head — both fixes, both at 25 iters / 50 steps / wrist+right, differing only in how the reach corridor is protected (**plane gate vs taper**) and in the DART ratios. *run 23 vs run 19* conflates DART with both fixes and is readable only on columns the fixes do not touch. <br>**Read:** `dart_scale` (NEW) is the mean taper over executed free jolts — near the 0.2 floor means most jolts fire close in and the taper is carrying the run. `dart/steps` should fall 0.268 → ~0.18 **by the ratio alone**, not to run 21's 0.209 which was jolts being lost. `success_rate` is directly comparable with run 19 iteration-for-iteration (mean 0.550 over iters 1–18, peak 0.743 @it25; **noise floor 0.088**). `EVAL.holdout` still false. ~14–16 h, request 20 h. |
...
(`*_off1034`); three change **only what is measured** and cost nothing upstream.

| key | run 11 | run 16 | changes `d`? |
|---|---|---|---|
| `SIM.anchor_update` | *(absent)* `latched` | `live` | yes |
| `SIM.anchor_hand_ref` | *(absent)* `wrist` | `hand_centroid` | yes |
| `SIM.d_point_depth` | `0.1122` | `0.1034` | yes |
...
Measured on `regrasp_pins_train_off.json`, moving to the 0.1034 midpoint turns
the direction a **median 9.9°** (p90 18.6, max 25.7), lengthens the median chord
3.89 → 4.11 cm, **re-bins 8.0% of the 1418 grasps**, and puts 45 of them under
`d_min_offset`. That is far too much to run against run 11's tables, and
`resolve_d_rule` was comparing only the rule NAME — it now compares depth and
refuses, quoting these numbers.

**The azimuth came from a point the robot does not have.** The anchor x axis is
`horizontal(c_object − p_giver)`, and `p_giver` was `mano.body.link_state[0, 7]`
— the MANO wrist JOINT, simulator ground truth. But
`my_regrasp_policy_runner._set_direction` uses `class_centroid(fused.hand_xyz)`,
the segmented hand POINT CLOUD's centroid. Different points: the joint sits at
the base of the palm, the cloud centroid out in the visible middle of the hand.
So runs 1–15 defined the frame one way in simulation and another on hardware.
`hand_centroid` makes both references come from the same cloud.

**The eval is stratified and covers every bin.** `linspace` over scenes made the
per-bin counts a by-product: run 11 got `+x 54, −x 33, +y 37, −y 25, +z 19,
−z 26`, and `−z` — the *largest* bin in the population at 283 pairs — was scored
on 26. At n=19 one `+z` episode is 5.3 points and the binomial SE is ±11, which
is most of what those curves show. `per_bin_frac` at 0.3 takes 30% of each bin's
pairs (≈428 in-table pairs over ≈318 scenes on the unpruned table), spread by
`np.linspace` so no seed is consumed and two runs are comparable episode by
episode.

On top of that, `full_bin_coverage` rolls every selected scene under **every**
bin, including the ones it never demonstrates. Those rows carry `in_table: 0`,
and the metric block reports both populations from the **same** rollouts:

- `succ_bin_b` — in-table rows only. Unchanged, comparable with runs 1–15.
- `succ_bin_all_b` — every row for that bin. What happens when the ladder asks
  for a direction this scene never showed the policy.

The gap between them is the generalisation-to-an-undemonstrated-direction
measurement, and nothing in the phase has ever produced one. It is also the
number that decides whether `k` is really a test-time knob. Both series are drawn
in one panel (solid / dotted, same colour per bin) in `curves_regrasp.png` and
`curves_regrasp2.png`. ⚠ It takes the eval from ~194 episodes to scenes × 6.

**The retry ladder is now ordered by success, and fixed.** `RETRY_LADDER =
(+x, +z, +y, −y, −z, −x)`, from run 11 iteration 22's per-bin rates
(`+x 0.778, +z 0.737, +y 0.649, −y 0.600, −z 0.538, −x 0.333`). Runs 1–15 walked
pin SLOT order, which is ascending bin index and arbitrary with respect to
quality — `−x`, the worst bin, was tried **second** whenever a scene had it.
Rung k is now a definite direction rather than a per-scene mixture (rung 1 was
`+x` on only 54% of run 11's scenes), so `retry_bin_frac_k` is 1.0 by
construction. The ladder is a constant, not re-fit per run: deriving it from the
run being scored would let `retry_at_k` improve by luckier ordering.

`retry_at_k` keeps its in-table population, so the reordering is the only thing
that changed about it. Two new columns make it readable: `retry_n_k` (how many
scenes actually offer k attempts) and `retry_at_k_deep` (the rate over those).
Without them `retry_at_4 = 0.75` looks broken next to `succ_bin_+x = 0.778` —
the resolution is that retry counts all 100 scenes while `succ_bin_+x` counts
the 54 that have a `+x` demo, and those are the easier ones.

**The frame was pinned to the worst view of the episode.** `d` lives in a frame
whose origin is the object's centroid, and every run through 15 built that frame
once, from the cloud observed at step 0, on the reasoning that the giver's hand
is static under `MANO_SIMULATION_MODE: disable_control_and_move_by_reset`. The
hand is static; the *observation* is not. The camera is eye-in-hand, so `c` is
the centre of the part of the object the gripper can currently see — at step 0 a
distant self-occluded slice, at the close a near view of the face the fingers are
on. The centroid moves through the episode while nothing in the scene does.

It is also the only version hardware can run.
`my_regrasp_policy_runner._set_direction` builds the frame from segmented camera
points and latches it too, so a deployed policy inherits whatever the first fused
cloud happened to see. `live` makes sim and rig compute the same quantity.

**What moves with it, and the risk.** The command is
`to_world(axes[bin], anchor_R)`, so a frame that moves re-issues a command that
moves — intended, and the hazard: the tracked target is no longer stationary
within an episode. Two things bound it. The anchor x axis is a *horizontal*
azimuth, so centroid error along gravity moves the frame not at all; and the
wrist→base fallback is hysteretic (0.04 m in, 0.08 m out) with one `AnchorState`
per episode, so the single discontinuity the frame can have cannot chatter.
`n_anchor_blind` counts steps whose cloud had no object points — there the
previous frame is kept rather than the command dropped, since zeroing both
direction channels reads to the network as a valid instruction, not a missing
one.

Shard compatibility is preserved: `anchor_R` and `centroid_world` still mean
step 0, and `anchor_R_final` / `centroid_world_final` carry the closing frame.
`bin_realized` and `d_grasp_world` are computed against the closing one, on the
grounds that the closest view is the best centroid estimate the episode ever had.

**The second change is scoring, not policy.** `d_min_offset: 0.02` is a
collection guard — it stops a fingertip landing on the centroid from minting a
confident direction out of noise. `_dir_block` was also applying it at
measurement time, where `d_rule.of` returns `None`, the episode is `continue`d,
and it leaves `dir_err`, `sector_err`, `bin_hit_rate` and the confusion matrix
entirely. The episodes removed are exactly the ones that ended up on the
centroid, which is a failure mode, so the metric was computed over the subset
that did not exhibit it. `dir_drop_short: false` measures them at `min_offset` 0;
`dir_n` and `dir_n_short` are logged so the size of that set is visible.

This one is separable after the fact — re-scoring run 11's rows with the flag off
isolates it offline, since it touches no data — so run 16 is not a clean
single-variable test but it decomposes into one.

**Where to look first.** Against run 11's `success 0.619 @it22`, a live frame
should help most where the step-0 view is worst, so read the per-bin split before
the aggregate: `−z` (from beneath) and `−x` (over the giver's fingers) have the
most occluded opening views and are the two bins `grasp_offset` unlocked in the
first place.

### regrasp_run15 — run 12 redone with the corrected dose and a warm start, not yet run

`examples/configs/regrasp_run15.yaml`. **Run 12's substrate with both of its
faults repaired and the initialisation changed**, six keys:

| | run 12 | run 15 |
|---|---|---|
| `dart_noise_ratio` | 1.0 → 90% of steps | **0.3** → 27% |
| `dart_alpha_scale` / `_end` | 1.0 → 0.25 | **0.25 → 0.12** |
| `train_from_scratch` | true (FTL) | **false** |
| `init_ckpt` | best *(inert)* | **last** *(live)* |
| `iter_epochs` | 15 | **20** |
| substrate | `_off`, `grasp_offset`, filter ON, m=600 | *identical* |

**It moves two axes, so on its own it settles neither.** Against run 12 both the
DART dose and the initialisation changed; a gap cannot be assigned to either.

**Its purpose is to pair with run 14.** The two carry the same corrected dose and
the same warm-started chain on two different substrates:

| | run 14 | run 15 |
|---|---|---|
| `d_rule` | `approach_axis` | `grasp_offset` |
| tables / shards | run 9's | `_off` |
| `reach_filter` | off | on |
| `episodes_per_iter` | 400 | 600 |
| dose, chain, epochs | — | *identical* |

An effect visible in both is a property of the intervention; in neither, the
hypothesis dies; in one only, it interacts with the substrate. That is the same
replication logic runs 12 and 13 were built on, and it is why `iter_epochs` is 20
here rather than run 12's 15 — matching run 14 is what makes the pair
single-variable. Setting it back to 15 gives the literal "run 12 plus two fixes"
instead, at the cost of that comparison.

**The α differs from run 14 deliberately.** α multiplies `tr(Σ̂₁)`, the first
iteration's *measured* learner error, and the two stacks measure different values:

| stack | anchor | α | applied trace |
|---|---|---|---|
| `_off` (runs 10/11/12/15) | 0.0408 | **0.25 → 0.12** | ~0.0102 |
| run 9's (runs 13/14) | 0.0287 | 0.35 → 0.18 | ~0.0102 |

Same executed magnitude — roughly [1.4, 1.5, 1.8] cm and [0.066, 0.054, 0.046]
rad in the free phase. Copying run 14's 0.35 onto this config would run it 1.4×
hot, which is the mistake run 12 made in the other direction.

**What run 12 actually did**, so this is read correctly: it finished 25 iterations
and regressed to 0.562 @it22 against run 11's 0.619, with `D` ending at 2147
episodes against 3660. The cause was the firing rate, not the magnitude —
`dart_noise` fires only on expert steps, so ratio 1.0 at β 0.9 perturbed 90% of
all steps against the jolt's 26%, the EE never converged inside
`reach_commit_dist: 0.05`, and the reach never committed. `c_env_done` actually
*improved*, 52 → 46.

**Read the guards at iteration 1, before `c_env_done` or `c_close_label`:**

| guard | run 11 (working) | run 12's failure |
|---|---|---|
| `c_max_steps` | ~21 / 190 | 92 |
| `reached_standoff` | ~129 / 190 | 59 |
| `mean_min_pos` | ~0.049 m | 0.131 |

If those move, the noise is again holding the EE outside the commit radius and
nothing downstream is interpretable — halve `dart_noise_ratio` before touching α.

Reach-tail DART is on: under `dart_noise` the reach is gated by
`dart_noise_ratio` (not `dart_reach_ratio`, which is jolt-only), Σ is scaled by
`diag(0.3005 ×3, 1.0 ×3)` inside the committed reach, and every draw is
clearance-rejected. Confirm `dart_reach` **and** `dart_reject` are both nonzero at
iteration 1 — `dart_reach` counted only jolt-path perturbations until the run-14
fix, so runs through 12 logged 0 there while reach noise was firing.

**Cost: ~9.5 h.** From run 11's 7.11 h on the same substrate with working
collection, 81% of it refit, plus 33% for the extra epochs, plus the base fit.
Not from run 12's 4.6 h — that was cheap only because its collection had
collapsed and `D` grew 41% too slowly. Request `14:00:00`.

The `_off` tables and shards, unchanged. `docs/runbook_run10_train.md` applies
with the config name swapped.

---

### regrasp_run14 — run 13's chain warm-started instead of refit, not yet run

`examples/configs/regrasp_run14.yaml`. **Run 13 with three keys changed, all in
TRAIN.** SIM and the entire DART block are byte-identical to run 13, so this is a
clean single-axis test of how the refit is initialised.
...
**What the chain does.** Iteration 0 fits the base demonstrations for
`base_epochs` from a random init, unchanged. From iteration 1 on,
`train_on_aggregate` is handed the previous iteration's run dir and loads
`checkpoints/last.pt` from it, so the chain is 0 → 1 → 2 → … → 25. Each link still
trains on the full aggregate `D = base ∪ D_1 ∪ … ∪ D_i`; the warm start changes
where the optimiser starts, not what it sees.

**The normalizer chains with the weights and is never recomputed**, and this is
not one of the three keys. On a warm start `train_on_aggregate` loads
`normalization.npz` from the run it is seeded from rather than recomputing stats
over the aggregate — the head's output scale is *defined* by the normalizer, so
warm-starting under freshly computed stats would silently reinterpret it. The
chain therefore pins iteration 0's statistics, computed over the base
demonstrations alone, for all 25 iterations, where run 13 recomputes them over a
growing `D` every time. Worth remembering before attributing any gap to the warm
start itself.

**Regrasp has already run this ablation, and it said "no effect".** Run 8 = run 7
+ `train_from_scratch: false`, one key, same tables and shards:

| | best | final | `dir_track` |
|---|---|---|---|
| run 7 (scratch) | **0.555** @it12 | it17 0.496 | 0.704 |
| run 8 (warm start) | 0.487 @it14 | it19 0.475 | 0.704 |

The warm start removed the FTL dip — no early trough, `train_loss` roughly half
run 7's (0.14 vs 0.235) — and bought nothing: success, `dir_track` and `bin_diag`
all at or below run 7's from iteration 7 on. The 0.068 gap sits **inside** that
setup's measured noise floor of 0.088, so run 8 is evidence of no effect rather
than of harm.

Two things make it worth asking again rather than treating run 8 as settled. Run 8
chained on `best.pt`; this chains on `last.pt`, the more converged fit and the
variant phase-4 run 20 used. And run 8 sat on the jolt, while this sits on
corrected `dart_noise`, where `D` grows differently — the FTL dip a warm start
removes is a real cost when every refit discards a working policy, and how much
that matters depends on what collection is actually contributing.

**One cost to know about.** `EVAL.ckpt` is `best`, so evaluation and the next
collection both run on `best.pt` while the weights carried forward are `last.pt`.
The chain and the data therefore come from two slightly different policies. Run 8
chose `best` precisely to keep them the same; if that turns out to matter, flipping
`init_ckpt` back is a one-word change.

**What to read**, against run 13 iteration for iteration:

- **The dip.** Run 13 should trough at iteration 1–2 and climb back above its base
  by iteration 5 (run 7 did exactly this: 0.370 → 0.244 → above base by 5). Run 14
  should not dip. That is the warm start working as advertised and is not by itself
  a reason to prefer it.
- **The ceiling, iterations 10–25.** Run 8's warning is that the chain plateaus
  lower. A gap over ~0.09 exceeds the noise floor and means the warm start costs.
- **`train_loss` and `val_loss` together.** A warm-started chain that memorises
  drives the first down while the second flattens or rises. Under FTL that
  divergence cannot happen the same way, which is what makes the pair — not
  `train_loss` alone — the diagnostic here.

`iter_epochs: 20` means something different under warm start: 20 *more* epochs on
top of everything the chain already learned, not a complete fit. By iteration 25
the earliest shards have been seen ~500 epochs and the newest 20.

Run 9's tables and shards, unchanged. Budget **~17–18 h** (run 13 ≈ 13.8 h, +33%
on the refit, which is ~85% of the wall clock) and request `22:00:00`.
...
columns of the same log say why:
...
`c_env_done` — the benchmark terminating the episode on human contact, a dropped
object, or timeout — is **~34% of collection episodes at every one of 25
iterations, with no downward trend**. `c_close_label`, the episodes that got to
the grasp, does not rise either; it drifts *down*, 143 → 114. So `D` stops
gaining states near the grasp at around iteration 5, and `eval_min_pos` flattens
at the same place because there is nothing closer in the aggregate to learn from.

**Why the jolt is the suspect.** Runs 1–11 call the perturbation DART but
*replace* the expert action with a uniform draw from a fixed box on 30% of steps.
Two properties make that destructive near the object: the arm stops tracking the
plan entirely for that step rather than wobbling around it, and the rotation
magnitude is **~2.5× the error it is meant to rehearse** — measured per-axis
learner discrepancy sd `rot [0.0836, 0.0590, 0.0676]` rad against the jolt's
uniform sd `0.1732`. Position is about right (`0.0143/0.0164/0.0245` vs `0.0231`)
but **anisotropic**: pos-z carries 1.7× the spread of pos-x, so a box draw spends
most of its noise in directions the learner is not wrong in.

**What `dart_noise` does instead.** Laskey et al. 2017: execute `π*(s) + ε`,
`ε ~ N(0, Σ)`, with Σ estimated each iteration from `(π_θ − π*)` outer products
(Eq. 3) and trace-rescaled to a fixed target (Eq. 4) anchored at `α ×` the first
iteration's measured error. Added rather than substituted, Gaussian rather than
uniform, measured rather than hand-tuned, shaped by where the learner is actually
wrong. α anneals 3.0 → 0.5, a 2.4× reduction in sd across the run. **The label is
unchanged in both modes** — the recorded action is always the clean `π*(s)` — and
that asymmetry is the mechanism.

Already implemented and exercised: phase-4 run 18 used it. Note run 18 is *not*
evidence either way on its own — it moved seven things at once and landed at 0.78
@it14 against run 16's 0.80. Here it is the only change.

**What would falsify it**, and it is checkable from iteration 3 in the
*collection* columns rather than the eval ones: `c_env_done` should fall below
~34% and `c_close_label` should rise. If neither moves, the perturbation was not
what was killing the episodes, and the next suspect is the OMG dive — 20% of base
episodes drop the wrist more than 5 cm below both endpoints mid-trajectory, which
is a planner problem no noise model reaches. The opposite failure is live too:
`dart_noise_ratio: 1.0` perturbs *every* supervisor step against the jolt's 30%,
...
### regrasp_run1 — the first Regrasp run (19 of 25, stopped by choice)

The run the phase started from, kept for the two details that make its curves
unreadable if you do not know them.

`m` was **354** for iterations 1–5 and **176** for 6–19 — a mid-run budget
correction, so **`D_steps` changes slope at iteration 6** and the |D| curve must
not be read as one trend. Base set **1087 episodes / 20386 steps** over 617
scenes (471 paired, 146 single). Evaluation and collection both read `best.pt`,
and the warm start read `best.pt` too, so the chain and the data came from the
same weights — the pairing run 8 later kept and run 14 deliberately breaks.

Run 2 is this run with the protocol fixed and two method changes, and is where
the phase actually starts: `success 0.143 → 0.563`.
...
| `regrasp_run19.yaml` | regrasp_run19 — = run 9 + `DAGGER.shield: true` and `TRAIN.iter_epochs` 15→20. **Two changes, not a clean ablation.** Ran 25/25, peak 0.743 @it25. Its shards carry the pin-table bug (18.4% of episodes on excluded pairs) — see Standing conclusions. |
| `regrasp_run20.yaml` | regrasp_run20 — = run 19 + `SIM.anchor_hand_ref: wrist → base` and nothing else; the single-variable test of the anchor frame. Full upstream rebuild under `_aabase`. Not yet run. |
| `regrasp_run21.yaml` | regrasp_run21 — = run 19 + **seven** changes: both bug fixes, `plane_gate`, `max_steps` 50→80 (both bands), `num_iters` 25→35 (and the beta ramp with it), and **right camera only**. Full upstream rebuild under `_right`. **Not a controlled run** — read run 22 against run 19 instead, and run 21 against run 22 for the other four. Pipeline: `regrasp_run21_all.sbatch`. |
| `regrasp_run22.yaml` | regrasp_run22 — = run 19 + **both bug fixes** + `DAGGER.plane_gate: true`, everything else byte-identical (wrist+right, 50 steps, 25 iters, same beta at every index). **The controlled run**: `success_rate` is directly comparable with run 19 iteration-for-iteration. Nothing to rebuild. |
| `regrasp_run23.yaml` | regrasp_run23 — = run 19 + **both bug fixes** + DART changes: ratios 0.3→0.2 on both bands, and `dart_taper_floor: 0.2` shrinking the free jolt from ±4 cm to ±0.8 cm as the standoff is approached. `plane_gate` OFF — **the taper replaces it**. Read against run 22: same fixes, same everything else, so the two differ only in how the reach corridor is protected. Nothing to rebuild. |
...
computes that mean from **whichever pin table it is handed**. Handed the test
table it returned the *test split's* centroids, which sit 2.9° off run 19's `+x`,
5.9° off its `-y` and 6.0° off its `+z`. Every held-out number scored before this
was measured against a command no part of the run ever issued, and nothing looked
wrong. `eval_regrasp_testset.py` now loads the run's own `command_axes.json` —
the same file the real-robot runner reads — and prints the offset it corrected.
`--split-axes` restores the old behaviour for the one case where it is the right
question. **Run 9's existing `test_log.csv` predates the fix.**

**A pin table is only valid for the camera rig that built it.** Its per-scene
`anchor_R` and `centroid_world` come from the observed cloud, so
`regrasp_pins_test.json` (built under `pretrain_multicam_wr.yaml`) cannot score
run 21 (trained under `pretrain_right.yaml`) — the same class of silent frame
mismatch that cost run 16, one axis over. `eval_regrasp_testset.py` now refuses
that pairing and prints the build commands. Runs 19, 22 and 23 match the existing
test table on all three of `cfg_file`, `d_rule` and `anchor_hand_ref`; run 21
needs `regrasp_pins_test_right.json` built first, with **no `--drop-bins`** —
its train table has five live bins, including `-x`.
...
**The reach filter never reached the workers — runs 10–20 collected on a pool
they were not configured to collect on.** Two prunes narrow the DAgger pool.
`demo_ok_table` is a FILE, so the manager and every worker read the same one.
The reach prune is DERIVED from the base shard and lived only in
`train_regrasp.py`; `build_regrasp_context` never applied it. Because
`keep_only` **renumbers slots** and the pool is shipped `(scene, slot)` rather
than the table, the two sides disagreed about what every slot index *meant*, not
merely about which pairs existed.

Measured on run 19: **246 of 1097 manager slots (22.4%)** named a different bin
in the worker, and **898 of 4883 episodes (18.4%)** landed on pairs the filter
had excluded. Those pairs failed `reached()` **77.1%** of the time against
**31.1%** for pairs that belonged in the pool — **692 of the run's 1933 dropped
episodes, 36% of all the collection waste**. `eval_jobs` ships slot indices the
same way, so `succ_bin_*` was mis-attributed on the same ~22% of slots, and the
standalone `eval_regrasp_run.py` / `eval_regrasp_testset.py` used the unpruned
numbering too.

**Affected: every run with `reach_filter` on and a parallel pool — runs 10, 11,
12, 15, 16, 17, 18, 19.** Runs 1–9 predate the default and are unaffected; a
`--num-workers 0` serial run is unaffected, since `collect_iteration` receives
the manager's table directly.

**What this does and does not invalidate.** The pooled `success_rate` of each run
is still a real measurement *of the policy that was actually trained* — but the
eval set is not the one the config describes, and per-bin numbers carry the wrong
labels on about a fifth of slots. Treat cross-run `succ_bin_*` comparisons in
this range as unreliable and the pooled rates as measurements of a slightly
different experiment than the config claims.

Scene 79 is the whole thing in one scene: its base demo for `+x` stopped 3.55 cm
short and its `+z` demo reached to 0.46 cm, yet over 25 iterations run 19
collected `+x` **three times** and `+z` **zero** — the manager's one-slot table
only ever emits slot 0, which is `+x` in the worker's two-slot table, so `+z` sat
at an index the manager could never emit. Fixed in `regrasp/setup.py` for run 21,
with no config key: there is no version of the loop that should keep it.
