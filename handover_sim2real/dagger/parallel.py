"""Parallel D_i collection for Phase 4 — the DAgger analog of rl/parallel_rollout.py.
...
DETERMINISM. The MANAGER picks the scenes (train_dagger_phase4's single rng,
unchanged) and stamps each job with its own seed, so the scene sequence is
...
round-robins, so the iteration takes ceil(m / W) episode-times. At m=100, W=20
gives 5 each (100% utilization) while W=16 gives 7 for some workers (89%). Past
...
def _worker_main(worker_id, cfg4, seed, worker_device, task_q, result_q):
    ...
    A job batch is (run_dir, ckpt, beta, params, iteration, [(scene, seed), ...]).
    ...
    try:
        ...
        from handover_sim2real.dagger.collector import collect_dagger_episode
        from handover_sim2real.dagger.policy_io import load_policy_runner
        from handover_sim2real.dagger.setup import build_phase4_context
        ...
        ctx = build_phase4_context(cfg4, seed=seed, verbose=False)
        ...
        while True:
            ...
            out = []
            for idx, scene, ep_seed in items:
...
class ParallelDaggerCollector:
    ...
    `collect(run_dir, ckpt, scenes, seeds, beta, params, iteration)` returns
    `[(episode | None, stats), ...]` in the order `scenes` was given — the same
    ...
    def collect(self, run_dir, ckpt, scenes, seeds, beta, params, iteration):
        if not scenes:
            return []
        jobs = list(zip(range(len(scenes)), scenes, seeds))
