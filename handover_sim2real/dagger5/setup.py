...
`examples/train_dagger_phase5.py` and `examples/eval_dagger_run_p5.py` both have to
...
call `build_phase5_context(cfg)` here. The eval scene set in particular is
...
from handover_sim2real.dagger5.env_setup import build_sim_cfg, build_sim_context
from handover_sim2real.dagger5.evaluator import EvalParams
from handover_sim2real.dagger5.grasp_box import build_box_params
from handover_sim2real.dagger5.grasp_pin import load_grasp_pin_table
from handover_sim2real.dagger5.pregrasp import forward_dist_default
...
def scene_pools(num_scenes: int, ev: dict,
    ...
    — which is what lets examples/eval_dagger_run_p5.py reproduce the in-loop pool.
    """
...
def build_phase5_context(cfg4: dict, *, seed: int = 0,
                         verbose: bool = True) -> Phase5Context:
    ...
    # eval_dagger_run_p5.py's reproduction of the in-loop pool are all unchanged.
    num_grasps = pin_table.num_grasps if pin_table is not None else 1
    if verbose:
        print(f"[phase5] {num_grasps} pinned grasp(s) per scene -> "
