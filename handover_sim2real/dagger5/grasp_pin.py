"""
Apply the per-scene grasp pin table (built by examples/build_grasp_pin_table_multi.py).
...
class GraspPinTable:
    ...
    def num_grasps(self) -> int:
        ...
        count — `select_pinned_grasps.py` drops scenes that cannot supply N — so
    ...
    def check_against(self, sim_cfg_block: dict) -> None:
        ...
        if want_split and was_split and want_split != was_split:
            raise ValueError(
                ...
                f"one per split (build_grasp_pin_table_multi.py --split {want_split}).")
