...
def collect_dagger_episode(sim, runner, scene_idx, *, rng,
                           ...
                           grasp_idx: int = 0):
    """Roll out `runner`'s policy on `(scene_idx, grasp_idx)`, labelling every
    visited state.
    ...
    Phase 5: `grasp_idx` selects which of the scene's pinned grasps this episode
    is aimed at, and the same index is handed to the learner as its conditioning
    (`runner.set_goal`). The two must agree — a policy conditioned on grasp A
    while the expert labels towards grasp B is the one bug that would look like
    "conditioning does not work" while actually being a wiring error, so the
    goal is set from the pinned pose read back off the planner rather than from
    the table.

    Returns (episode | None, stats). `episode` is None when the pair is unusable
    (OMG cannot plan at step 0, or the pin does not match) or nothing was
    recorded.
    """
    env = sim.env
    ...
    if params.derive_standoff and pin_table is not None:
        pinned_pose = pin_table.pose(int(scene_idx), int(grasp_idx))
        if pinned_pose is not None:
            standoff_pose = derived_standoff_pose(
                pinned_pose, params.standoff_dist, params.reach_tail)
    ...
    for step in range(params.max_steps):
        ...
        if committed_reach is None:
            ...
            if plan is None:
                ...
                #
                # Phase 5: which grasp is chosen by `grasp_idx`, and the pinned
...
class DaggerHDF5Writer:
    ...
    def append(self, episode: dict) -> None:
        ...
        # Phase 5 — written on every shard so the aggregate is uniform with the
        # base set and BCDataset can read the conditioning off the episode.
...
def collect_iteration(sim, runner, pairs, out_path, *, rng,
                      beta: float, params: CollectParams, pin_table=None,
                      registry=None, iteration: int = 0,
                      progress_every: int = 5, results=None) -> dict:
    """Collect D_i: `len(pairs)` trajectories under the current mixed policy.

    Phase 5: `pairs` is a list of `(scene_idx, grasp_idx)` tuples, not scene ids
    — the unit of work is a scene under a specific pinned grasp. A bare list of
    ints is still accepted and read as `(scene, 0)`, so a Phase-4-shaped call
    keeps working.
    ...
    """
    pairs = [(int(p), 0) if np.isscalar(p) or isinstance(p, (int, np.integer))
    ...
    writer = DaggerHDF5Writer(out_path, attrs={
        ...
        "scenes": np.asarray([s for s, _ in pairs], dtype=np.int32),
        # Phase 5 — the other half of the work unit. Parallel to `scenes`, so the
        # shard records exactly which (scene, grasp) pairs it holds.
        "grasp_indices": np.asarray([g for _, g in pairs], dtype=np.int32),
        "grasps_per_scene": int(getattr(pin_table, "num_grasps", 1) or 1),
    ...
    try:
        for i, (scene, grasp_i) in enumerate(pairs):
            ...
            if (i + 1) % progress_every == 0 or i == len(pairs) - 1:
                print(f"    [{i+1:3d}/{len(pairs)}] episodes={agg['episodes']} "
